  ...
  const [table, setTable] = useState({
    ...
    columns: [
      ...
      },
      {
        ...
        width: "120px",
        hide: false,
        renderCell: (params: any) => format(params.date, "dd.MM.yyyy"),
      },
      {
        field: "comment",
        ...
        <div className="flex items-center gap-[20px]">
          <FormControl size={"small"}>
            ...
            >
              ...
              {!expensesTypeList.loading &&
                !expensesTypeList.error &&
                expensesTypeList.result?.data.map(
                  (expense_type: any, index: number) => (
                    <MenuItem key={index} value={expense_type.id}>
                      {expense_type.name}
                    </MenuItem>
                  )
                )}
                ...
                >
                  {isLoading ? (
                    <div className="flex justify-center">
                      <div className="w-[19px] h-[19px] border-2 rounded-full border-white border-t-transparent animate-spin"></div>
                    </div>
                  ) : (
                    <div>
                      {modal.action === "delete" && "Удалить"}
