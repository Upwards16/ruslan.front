...
export default function EmployeesPage() {
  const userPosition =
    typeof getCookie(position) !== "undefined" ? getCookie(position) : "";
    ...
    for (const key in accessRules) {
      if (location.pathname.includes(key)) {
        rule = accessRules[key].find(
          (position: any) => position.position === userPosition
        );
      }
    }
  ...
  const [table, setTable] = useState({
    ...
    columns: [
      ...
      {
        field: "date_start_work",
        headerName: "Дата найма",
        width: "120px",
        renderCell: (params: any) =>
          format(params.date_start_work, "dd.MM.yyyy"),
      },
      {
        field: "status",
        headerName: "Статус",
  ...
  const getEmployee = async (params: any, action: string) => {
    let cv = new File([""], "");
    if (action !== "delete") {
      cv = await CommonService.convertImageUrlToFile(params.cv).catch(
        () => new File([""], "")
      );
    }
    setModal({
      ...
      values: {
        ...
        date_of_birth: params.date_of_birth
          ? dayjs(params.date_of_birth)
          : null,
        date_start_work: params.date_start_work
          ? dayjs(params.date_start_work)
          : null,
      },
    });
  };
        ...
        <div className="flex items-center gap-[20px]">
          <FormControl size={"small"}>
            ...
            >
              ...
              {!positionList.loading &&
                !positionList.error &&
                positionList.result?.data.map(
        ...
        children={
          <div className="flex flex-col text-[#505050]">
            <h1 className="text-[24px] font-bold text-center">
              ...
              {modal.action === "delete" &&
                `Удалить сотрудника ${modal.values.firstname} ${modal.values.lastname}?`}
            </h1>
              ...
              {modal.action !== "delete" ? (
                <div className="grid grid-cols-2 gap-[30px] gap-x-[60px]">
                  ...
                  <DatePicker
                    ...
                    value={
                      modal.values.date_of_birth
                        ? dayjs(modal.values.date_of_birth)
                        : null
                    }
                    onChange={(date) => {
                      setModal({
                  ...
                  <DatePicker
                    ...
                    value={
                      modal.values.date_start_work
                        ? dayjs(modal.values.date_start_work)
                        : null
                    }
                    onChange={(date) => {
                      setModal({
                  ...
                  <TextField
                    ...
                    onChange={(event: any) => {
                      setModal({
                        ...
                        values: {
                          ...modal.values,
                          hourly_payment_cost: InputCheckForNumbers(
                            event.target.value
                          ),
                        },
                      });
