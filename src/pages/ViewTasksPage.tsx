...
export default function ViewTasksPage() {
    ...
    if (!taskColumnsList.loading && !taskColumnsList.error) {
      ...
      if (id !== "all") {
        const fetchData = async () => {
          ...
          setTaskColumns(
            taskData.reduce((o, column) => ({ ...o, [column.id]: column }), {})
          );
        };
        fetchData();
      } else {
        const fetchData = async () => {
          return TasksService.GetUserTaskList().then((res) => {
            const tasks = res.data;

            let columns = data.map((col: any) => {
              return {
                ...col,
            ...
            setTaskColumns(
              columns.reduce(
                (o: any, column: any) => ({ ...o, [column.id]: column }),
                {}
              )
            );
          });
  ...
  const getTask = async (params: any, action: string) => {
    ...
    if (action !== "delete") {
      attachment = await CommonService.convertImageUrlToFile(
        params.attachment
      ).catch(() => new File([""], ""));
    }
    setModal({
      ...
      {taskColumnsList.loading ? (
        ...
        <div className="w-full flex justify-start items-start gap-[20px] overflow-x-scroll py-[40px]">
          <div className="w-full h-full flex justify-start items-start gap-[30px]">
            <DragDropContext
              onDragEnd={(result) =>
                onDragEnd(result, taskColumns, setTaskColumns)
              }
            >
              {Object.entries(taskColumns).map(
                ([columnId, column]: [string, any]) => {
                  return (
                    ...
                    >
                      <h3 className="text-[16px] font-[700] p-[20px]">
                        {column.name}
                      </h3>
                                        ...
                                        return (
                                          ...
                                          >
                                            ...
                                            <div className="w-full flex justify-between items-end">
                                              <div className="flex px-[4px] py-[2px] items-center rounded-[4px] bg-[#4e54e10f] gap-[6px]">
                                                <AccessTimeIcon
                                                  style={{
                                                    width: "14px",
                                                    height: "14px",
                                                    color: getColorsForDeadline(
                                                      task.deadline
                                                    ),
                                                  }}
                                                />
            ...
            {modal.action !== "view" ? (
                  ...
                  <div className="grid grid-cols-2 gap-[30px] gap-x-[60px]">
                    ...
                    <FormControl>
                      ...
                      >
                        ...
                        {!markList.loading &&
                          !markList.error &&
                          markList.result?.data.map(
                            (mark: any, index: number) => (
                              <MenuItem key={index} value={mark.id}>
                                {mark.name}
                              </MenuItem>
                            )
                          )}
                    ...
                    <DatePicker
                      ...
                      value={
                        modal.values.deadline
                          ? dayjs(modal.values.deadline)
                          : null
                      }
                      onChange={(date) => {
                        setModal({
                    ...
                    <FormControl>
                      ...
                      >
                        ...
                        {!taskParticipantsList.loading &&
                          !taskParticipantsList.error &&
                          taskParticipantsList.result?.data.map(
                  ...
                  >
                    {isLoading ? (
                      <div className="flex justify-center">
                        <div className="w-[19px] h-[19px] rounded-full animate-spin border-2 border-solid border-white border-t-transparent"></div>
                      </div>
                    ) : (
                      <>
              ...
              <div className={"pt-[40px] w-full flex flex-col gap-[30px]"}>
                <div className="w-full flex items-center gap-[20px]">
                  <div className="py-[2px] px-[10px] rounded-[4px] bg-[#FAFAFA] text-[10px] font-[500]">
                  ...
                  <div className="flex flex-col justify-start items-start gap-[14px]">
                    ...
                    <div className="flex items-center gap-[10px] h-[30px]">
                      <p className="text-[10px] font-[500]">
                        {
                          taskColumns[modal.values.column]?.tasks.find(
                            (task: any) => task.id === modal.values.id
                          ).mark?.name
                        }
                      </p>
                      <div
