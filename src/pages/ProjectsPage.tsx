...
export default function ProjectsPage() {
  ...
  const [table, setTable] = useState({
    ...
    columns: [
      ...
      },
      {
        ...
        width: "120px",
        renderCell: (params) => format(params.start_at, "dd.MM.yyyy"),
      },
      {
        field: "end_at",
        headerName: "Дата окончания",
        width: "120px",
        renderCell: (params) => format(params.end_at, "dd.MM.yyyy"),
      },
      { field: "cost", headerName: "Сумма", width: "120px" },
      {
        ...
        renderCell: (params: any) =>
          params.terms_of_reference !== null && (
            <IconButton
              onClick={() => {
  ...
  const getProject = async (params: any, action: string) => {
    ...
    let banner = new File([""], "");
    if (action !== "delete") {
      agreement = await CommonService.convertImageUrlToFile(
        params.agreement
      ).catch(() => new File([""], ""));
      terms_of_reference = await CommonService.convertImageUrlToFile(
        params.terms_of_reference
      ).catch(() => new File([""], ""));
      banner = await CommonService.convertImageUrlToFile(params.banner).catch(
        () => new File([""], "")
      );
    }
    setModal({
              ...
              {modal.action !== "delete" && (
                <div className="grid grid-cols-2 gap-[30px] gap-x-[60px]">
                  ...
                  <DatePicker
                    ...
                    value={
                      modal.values.start_at
                        ? dayjs(modal.values.start_at)
                        : null
                    }
                    onChange={(date) => {
                      setModal({
                  ...
                  <DatePicker
                    ...
                    value={
                      modal.values.end_at ? dayjs(modal.values.end_at) : null
                    }
                    onChange={(date) => {
                      setModal({
                        ...modal,
                ...
                >
                  {isLoading ? (
                    <div className="flex justify-center">
                      <div className="w-[19px] h-[19px] border-2 border-white border-t-transparent animate-spin rounded-full"></div>
                    </div>
                  ) : (
                    <div>
