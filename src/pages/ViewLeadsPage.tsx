...
export default function ViewLeadsPage() {
  const { id } = useParams();
  const lead = LeadsService.GetLeadNyAsyncHook(
  ...
  const handleCallDelete = (call: any, index: any) => {
    LeadsService.DeleteCall(call.id).then(() => {
      const newArray = [
        ...callList.slice(0, index),
        ...callList.slice(index + 1),
      ];
  ...
  useEffect(() => {
    if (!lead.loading && !lead.error) {
      ...
      setCallList(() =>
        data.call_history.map((call: any) => ({
          id: call.id,
          date: call.date,
          comment: call.comment,
          edit: false,
        ...
        <div className="grid grid-cols-2 gap-[24px]">
          <div className="p-5 rounded-[10px] bg-white shadow-block relative col-start-1 col-end-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-[10px] font-medium text-[#8C8C8C]">Лид</p>
            ...
            <div className="mt-[24px] flex justify-between">
              <div>
                ...
                <p className="text-[16px] text-[#505050] font-bold">
                  {lead.result?.data.user
                    ? `${lead.result?.data.user.firstname} ${lead.result?.data.user.lastname}`
                    : ""}
                </p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-[16px] text-[#505050] font-medium">
                  {moment(lead.result?.data.reminder_date).format(
                    "DD.MM.YYYY HH:mm"
                  )}
                </p>
              </div>
              <div>
                ...
                <p className="text-[16px] text-[#505050] font-medium">
                  {lead.result?.data.traffic_source
                    ? lead.result?.data.traffic_source.name
                    : ""}
                </p>
              </div>
              <div>
                <p className="text-[10px] font-medium text-[#8C8C8C]">Статус</p>
                <p className="text-[16px] text-[#505050] font-medium">
                  {lead.result?.data.status
                    ? lead.result?.data.status.name
                    : ""}
                </p>
              </div>
              <div>
                <p className="text-[10px] font-medium text-[#8C8C8C]">Номер</p>
                <p className="text-[16px] text-[#505050] font-medium">
          ...
          <div className="relative w-full h-full p-5 rounded-[10px] bg-white shadow-block flex flex-col justify-start items-start gap-[50px]">
            <h4 className="text-[16px] text-[#505050] font-bold">
              История звонков
            </h4>
                      ...
                      <td>
                        <div className="flex gap-[20px] h-[30px]">
                          ...
                          {call.edit ? (
                            ...
                            <IconButton
                              onClick={() => handleCallDelete(call, index)}
                            >
                              <DeleteIcon />
                            </IconButton>
                          )}
          ...
          <div className="relative w-full h-full p-5 rounded-[10px] bg-white shadow-block flex flex-col justify-start items-start gap-[50px]">
            ...
            <div className="w-full flex flex-col justify-start items-start gap-[20px] h-[250px] overflow-scroll border p-[10px]">
              <p className="w-full h-[228px] outline-none resize-none">
                {lead.result?.data.comment}
              </p>
            </div>
          </div>
        </div>
